...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if not explicit:
        try:
            ...
            if selection is not None:
                ...
                return sorted({*selection, "project", "conversation_branches"})
        except Exception:
            pass
    ...
    try:
        ...
        return sorted(enabled | {"project", "conversation_branches"})
    except Exception:
        if fallback_notice is not None:
            print(
...
def _(rid, params: dict) -> dict:
    ...
    try:
        # Resolve and validate credentials/provider once without mutating a live
        # agent. The picker itself is the explicit confirmation for this batch.
        ...
        # "All conversations" also defines the starting model for future chats
        # in this profile. Existing sibling model settings remain untouched.
        _persist_model_assignment(
            model=resolved["value"],
            provider=resolved["provider"],
        ...
        for stored_id, runtimes in live_by_stored_id.items():
            ...
            for runtime_id, session in runtimes:
                ...
                try:
                    ...
                    active_failed = active_failed or is_active
                    logger.warning(
                        "batch model switch failed for stored session %s: %s",
                        stored_id,
                        exc,
                    )
                finally:
...
def _(rid, params: dict) -> dict:
    ...
    if session.get("_delete_review_pending"):
        return _err(rid, 4042, "delete review is pending; new messages are disabled")
    session_key = str(session.get("session_key") or "").strip()
    db = _get_db()
    if db is not None and session_key:
        pending_getter = getattr(db, "get_pending_branch_merge", None)
        if pending_getter is not None and pending_getter(session_key) is not None:
            return _err(
                rid,
                4041,
                "branch merge is queued; new messages are disabled until it completes",
            )
    ...
    with session["history_lock"]:
        # A model switch is a short critical section between turns.  Queue the
        # prompt rather than starting it against a half-replaced agent client.
        if session.get("model_switching"):
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        completion_text = "branch review did not complete"
        ...
        try:
            ...
            with session["history_lock"]:
                ...
                session["last_active"] = time.time()
                _clear_inflight_turn(session)
                # Claim the hand-off before releasing the lock, so a new
                # prompt cannot start between the end of this turn and its
                # deferred model switch.
                if session.get("pending_model_switch"):
                    session["model_switching"] = True
                    apply_queued_model = True
            _apply_pending_branch_merges(
                str(session.get("session_key") or ""),
                claim_already_held=True,
                safe_live_sid=sid,
        ...
        # The waiting merge RPC owns the post-review injection/deletion. Do not
        # drain queued prompts or start automatic continuations on a branch that
