...
TASK_STATUS_GUIDANCE = (
    ...
    "Rules:\n"
    "- background: the WHOLE conversation's big-picture task — one sentence "
    "restating what this session is working toward overall (the stable "
    "session-level context, e.g. '处理面板分割G8脚本'), NOT the tiny ask of "
    "the latest turn. (整个对话的大背景, 不是当前这一轮的小问题)\n"
    "- progress: where the overall task stands right now — one sentence on "
    "how far the whole task has advanced (the session-level progress), NOT "
    "what this single reply did. (整个大背景下的当前进度, 不是这一轮回复的进度)\n"
    "- next: the immediate next action to advance the WHOLE task. "
    "(推动整个任务前进的下一步)\n"
